...
for name, model in models.items():
    cv_mean, cv_std = cross_validate_model(model, X_train_tfidf, y_train, cv=5, scoring='accuracy')
    results = evaluate_model_optimized(model, X_train_tfidf, X_test_tfidf, y_train, y_test, all_classes, calc_auc=True)
    display_results(
        model_name=name,
        accuracy=results[0],
        precision=results[1],
